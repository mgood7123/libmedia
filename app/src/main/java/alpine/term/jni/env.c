...
env_t env__new() {
    env_t envempty = (env_t) malloc(1 * sizeof(*envempty));
    envempty[0] = NULL;
    return envempty;
}
...
env_t env__add(env_t env, const char *string) {
    ...
    env_t array_tmp;
    int i = env__size(env);
    array_tmp = (env_t) realloc(env, ((i + 2) * sizeof(char*)));
    if (array_tmp == NULL) {
...
env_t env__add_env(env_t env, char *name, env_t *variable) {
    ...
    size_t len = 1+strlen(name)+strlen("=")+strlen("0x")+(sizeof(variable)*2); /* (0x00 to 0xFF) == 1 byte */
        ...
        if (s)
        {
            ...
            while (pch != NULL)
            {
                ...
                if (strcmp(pch,string) != 0 ) {
                    ...
                    char * tmp = (char *) malloc(strlen(pch) + strlen("=") + strlen(string2));
                    tmp = strcatb(tmp, pch);
                    tmp = strcatb(tmp, "=");
...
/*                     	printf("appending %s to %s\n\n", string, s); */
                    char * tmp = (char *) malloc(strlen(s) + strlen(string));
                    tmp = strcatb(tmp, s);
