...
public final class TerminalView extends View {
    ...
    public TerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                ...
                if (!mEmulator.isMouseTrackingActive()) {
                    if (!e.isFromSource(InputDevice.SOURCE_MOUSE)) {
                        if (mClient != null) mClient.onSingleTapUp(e);
                        return true;
                    }
                }
                return false;
            ...
            public void onLongPress(MotionEvent e) {
                ...
                if (mClient != null) if (mClient.onLongPress(e)) return;
                if (!mIsSelectingText) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (mClient != null) if (mClient.onKeyDown(keyCode, event, mTermSession)) {
            invalidate();
            return true;
        }

        // the above statment returns true if it succeeds so why even bother having an else if
        ...
        boolean controlDown = event.isCtrlPressed();
        boolean leftAltDown = (metaState & KeyEvent.META_ALT_LEFT_ON) != 0;
        if (mClient != null) {
            if (!controlDown) controlDown = mClient.readControlKey();
            if (!leftAltDown) leftAltDown = mClient.readAltKey();
        }
    ...
    public void inputCodePoint(int codePoint, boolean controlDownFromEvent, boolean leftAltDownFromEvent) {
        ...
        if (mClient != null) {
            if (!controlDown) controlDown = mClient.readControlKey();
            if (!altDown) altDown = mClient.readAltKey();
    ...
    private void startTextSelectionMode() {
        ...
        if (mClient != null) mClient.copyModeChanged(mIsSelectingText);
        invalidate();
    }
