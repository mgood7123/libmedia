...
extern void
mobileffmpeg_log_callback_function(void *ptr, int level, const char *format, va_list vargs);
...
int show_codecs(void *optctx, const char *opt, const char *arg) {
    ...
    for (i = 0; i < nb_codecs; i++) {
        ...
        av_log(NULL, AV_LOG_STDERR, " %-20s %s", desc->name,
...
static void print_codecs(int encoder) {
    ...
    for (i = 0; i < nb_codecs; i++) {
        ...
        while ((codec = next_codec_for_id(desc->id, codec, encoder))) {
            ...
            av_log(NULL, AV_LOG_STDERR, " %-20s %s", codec->name,
...
int show_layouts(void *optctx, const char *opt, const char *arg) {
    ...
    for (i = 0; !av_get_standard_channel_layout(i, &layout, &name); i++) {
        if (name) {
            ...
            for (j = 1; j; j <<= 1)
                if ((layout & j))
                    av_log(NULL, AV_LOG_STDERR, "%s%s", (layout & (j - 1)) ? "+" : "",
