...
build_application_mk() {
  ...
  if [[ ${ENABLED_LIBRARIES[$LIBRARY_X265]} -eq 1 ]] || [[ ${ENABLED_LIBRARIES[$LIBRARY_TESSERACT]} -eq 1 ]] || [[ ${ENABLED_LIBRARIES[$LIBRARY_OPENH264]} -eq 1 ]] || [[ ${ENABLED_LIBRARIES[$LIBRARY_SNAPPY]} -eq 1 ]] || [[ ${ENABLED_LIBRARIES[$LIBRARY_RUBBERBAND]} -eq 1 ]]; then
    ...
    ${SED_INLINE} 's/c++_shared //g' ${BASEDIR}/android/jni/Android.mk
  fi
...
# DETECT BUILD TYPE
rm -f ${BASEDIR}/android/jni/Android.mk
rm -f ${BASEDIR}/android/app/build.gradle
if [[ ! -z ${BUILD_LTS} ]]; then
  enable_lts_build
  BUILD_TYPE_ID+="LTS "

  cp ${BASEDIR}/tools/ndk/Android.lts.mk ${BASEDIR}/android/jni/Android.mk
  cp ${BASEDIR}/tools/release/android/build.lts.gradle ${BASEDIR}/android/app/build.gradle
else
  cp ${BASEDIR}/tools/ndk/Android.mk ${BASEDIR}/android/jni/Android.mk
  cp ${BASEDIR}/tools/release/android/build.gradle ${BASEDIR}/android/app/build.gradle
...
echo -e -n "INFO: Building mobile-ffmpeg ${BUILD_VERSION} ${BUILD_TYPE_ID}library for Android: "
echo -e $(date)
...
if [[ -n ${ANDROID_ARCHITECTURES} ]]; then
  ...
  ./gradlew app:clean app:assembleRelease app:testReleaseUnitTest
  ...
  cp ${BASEDIR}/android/app/build/outputs/aar/mobile-ffmpeg-release.aar ${MOBILE_FFMPEG_AAR}/mobile-ffmpeg.aar
