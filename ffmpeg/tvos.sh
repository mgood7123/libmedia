...
create_static_framework() {
  ...
  cp ${BASEDIR}/prebuilt/tvos-universal/$1-universal/lib/$2 ${FRAMEWORK_PATH}/$1
...
if [[ -f ${XCODE_FOR_MOBILE_FFMPEG} ]]; then
  source ${XCODE_FOR_MOBILE_FFMPEG}
fi
...
echo -e -n "INFO: Building mobile-ffmpeg ${BUILD_VERSION} ${BUILD_TYPE_ID}for tvOS: "
echo -e $(date)
...
if [[ ! -z ${TARGET_ARCH_LIST} ]]; then
  ...
  rm -rf ${BASEDIR}/prebuilt/tvos-universal
  mkdir -p ${BASEDIR}/prebuilt/tvos-universal
  rm -rf ${BASEDIR}/prebuilt/tvos-framework
  mkdir -p ${BASEDIR}/prebuilt/tvos-framework
  ...
  for library in {0..41}; do
    if [[ ${ENABLED_LIBRARIES[$library]} -eq 1 ]]; then
      ...
      if [[ ${LIBRARY_LIBTHEORA} == $library ]]; then
        ...
        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libtheora-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libtheoraenc-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libtheoradec-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libtheora.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libtheoraenc.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libtheoradec.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi
        ...
        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libvorbisfile-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libvorbisenc-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libvorbis-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libvorbisfile.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libvorbisenc.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libvorbis.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi
        ...
        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libwebpmux-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libwebpdemux-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libwebp-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libwebpmux.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libwebpdemux.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libwebp.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi
      ...
      then
        ...
        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libopencore-amrwb-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libopencore-amrnb-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libopencore-amrwb.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libopencore-amrnb.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi
        ...
        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libnettle-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/libhogweed-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libnettle.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/libhogweed.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi
        ...
        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-universal/${library_name}-universal/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi

        $(cp $(get_external_library_license_path ${library}) ${BASEDIR}/prebuilt/tvos-framework/${library_name}.framework/LICENSE)
        if [ $? -ne 0 ]; then
          echo -e "failed\n"
          exit 1
        fi
  ...
  cp -r ${BASEDIR}/prebuilt/tvos-${TARGET_ARCH_LIST[0]}-apple-darwin/ffmpeg/include/* ${FFMPEG_UNIVERSAL}/include
  cp ${BASEDIR}/prebuilt/tvos-${TARGET_ARCH_LIST[0]}-apple-darwin/ffmpeg/include/config.h ${FFMPEG_UNIVERSAL}/include
  ...
  for FFMPEG_LIB in ${FFMPEG_LIBS}; do
    ...
    cp ${FFMPEG_UNIVERSAL}/lib/${FFMPEG_LIB}.${BUILD_LIBRARY_EXTENSION} ${FFMPEG_LIB_FRAMEWORK_PATH}/${FFMPEG_LIB}

    # COPY THE LICENSES
    if [ ${GPL_ENABLED} == "yes" ]; then

      # GPLv3.0
      cp ${BASEDIR}/LICENSE.GPLv3 ${FFMPEG_LIB_FRAMEWORK_PATH}/LICENSE
    else

      # LGPLv3.0
      cp ${BASEDIR}/LICENSE.LGPLv3 ${FFMPEG_LIB_FRAMEWORK_PATH}/LICENSE
    fi
  ...
  rm -rf ${MOBILE_FFMPEG_FRAMEWORK_PATH}
  mkdir -p ${MOBILE_FFMPEG_FRAMEWORK_PATH}/Headers
  mkdir -p ${MOBILE_FFMPEG_FRAMEWORK_PATH}/Modules
  ...
  cp -r ${BASEDIR}/prebuilt/tvos-${TARGET_ARCH_LIST[0]}-apple-darwin/mobile-ffmpeg/include/* ${MOBILE_FFMPEG_UNIVERSAL}/include
  cp -r ${MOBILE_FFMPEG_UNIVERSAL}/include/* ${MOBILE_FFMPEG_FRAMEWORK_PATH}/Headers
  cp ${MOBILE_FFMPEG_UNIVERSAL}/lib/libmobileffmpeg.${BUILD_LIBRARY_EXTENSION} ${MOBILE_FFMPEG_FRAMEWORK_PATH}/mobileffmpeg

  # COPY THE LICENSES
  if [ ${GPL_ENABLED} == "yes" ]; then
    cp ${BASEDIR}/LICENSE.GPLv3 ${MOBILE_FFMPEG_UNIVERSAL}/LICENSE
    cp ${BASEDIR}/LICENSE.GPLv3 ${MOBILE_FFMPEG_FRAMEWORK_PATH}/LICENSE
  else
    cp ${BASEDIR}/LICENSE.LGPLv3 ${MOBILE_FFMPEG_UNIVERSAL}/LICENSE
    cp ${BASEDIR}/LICENSE.LGPLv3 ${MOBILE_FFMPEG_FRAMEWORK_PATH}/LICENSE
  fi
